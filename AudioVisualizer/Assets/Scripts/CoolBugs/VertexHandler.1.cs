...
public class VertexHandler1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        */
        ...
        for(int i = 0; i < layers; i++){
            if(vertices[i][0].z < 0){
                for(int j = 0; j < vertPerLayer; j++){
                    currentRad = MainAudio._audioBandBuffer[j] + initRad;
                    currentAngle = j * Mathf.PI*2f / vertPerLayer  + i*0.1f;
                    vertices[i][j] = new Vector3(vertices[i][j].x + Mathf.Cos(currentAngle)*currentRad, vertices[i][j].y + Mathf.Sin(currentAngle) * currentRad, layers * layerDist);
                }
            }

            for(int j = 0; j < vertPerLayer; j++){
                print(MainAudio._audioBandBuffer[j]);
                currentAngle = j * Mathf.PI*2f / vertPerLayer  + i*0.1f;
                vertices[i][j] = new Vector3(
                    vertices[i][j].x + Mathf.Cos(currentAngle) * MainAudio._audioBandBuffer[j] * 1000f,
                    vertices[i][j].y + Mathf.Sin(currentAngle) * MainAudio._audioBandBuffer[j] * 1000f,
